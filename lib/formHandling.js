  ...
  return new Promise((resolve, reject) => {
    ...
    form.parse(req)
      .on('fileBegin', (name, file) => {
        ...
        debug('INCOMING_REQUEST_FILE_BEGIN', name, file.name, file.type);
        file.extension = path.extname(file.name) || '.png';
        file.path = `/tmp/screenshot-${uniqueName}${file.extension}`;
      })
      .on('field', (name, field) => {
      ...
      .on('file', (name, file) => {
        ...
        debug('INCOMING_REQUEST_FILE', name, uniqueName, file.extension);
